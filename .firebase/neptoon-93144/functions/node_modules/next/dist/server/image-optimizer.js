...
function setResponseHeaders(req, res, url, etag, contentType, isStatic, xCache) {
...
function sendResponse(req, res, url, extension, buffer, isStatic, xCache) {
    const contentType = (0, _serveStatic).getContentType(extension);
    const etag = getHash([
        buffer
    ]);
    const result = setResponseHeaders(req, res, url, etag, contentType, isStatic, xCache);
    if (!result.finished) {
        res.end(buffer);
    }
}
