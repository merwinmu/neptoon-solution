body {
  font-family: system-ui, -apple-system,
    /* Firefox supports this but not yet `system-ui` */ 'Segoe UI', Roboto,
    Helvetica, Arial, sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji';
}

/**
Use a better box model (opinionated).
*/

.tina-tailwind *,
.tina-tailwind ::before,
.tina-tailwind ::after {
	box-sizing: border-box;
}

/**
Use a more readable tab size (opinionated).
*/

.tina-tailwind html {
	-moz-tab-size: 4;
	tab-size: 4;
}

/**
1. Correct the line height in all browsers.
2. Prevent adjustments of font size after orientation changes in iOS.
*/

.tina-tailwind html {
	line-height: 1.15; /* 1 */
	-webkit-text-size-adjust: 100%; /* 2 */
}

/**
Remove the margin in all browsers.
*/

.tina-tailwind body {
	margin: 0;
}

/**
Improve consistency of default fonts in all browsers. (https://github.com/sindresorhus/modern-normalize/issues/3)
*/

.tina-tailwind body {
	font-family:
		system-ui,
		-apple-system, /* Firefox supports this but not yet `system-ui` */
		'Segoe UI',
		Roboto,
		Helvetica,
		Arial,
		sans-serif,
		'Apple Color Emoji',
		'Segoe UI Emoji';
}

/**
1. Add the correct height in Firefox.
2. Correct the inheritance of border color in Firefox. (https://bugzilla.mozilla.org/show_bug.cgi?id=190655)
*/

.tina-tailwind hr {
	height: 0; /* 1 */
	color: inherit; /* 2 */
}

/**
Add the correct text decoration in Chrome, Edge, and Safari.
*/

.tina-tailwind abbr[title] {
	text-decoration: underline dotted;
}

/**
Add the correct font weight in Edge and Safari.
*/

.tina-tailwind b,
.tina-tailwind strong {
	font-weight: bolder;
}

/**
1. Improve consistency of default fonts in all browsers. (https://github.com/sindresorhus/modern-normalize/issues/3)
2. Correct the odd 'em' font sizing in all browsers.
*/

.tina-tailwind code,
.tina-tailwind kbd,
.tina-tailwind samp,
.tina-tailwind pre {
	font-family:
		ui-monospace,
		SFMono-Regular,
		Consolas,
		'Liberation Mono',
		Menlo,
		monospace; /* 1 */
	font-size: 1em; /* 2 */
}

/**
Add the correct font size in all browsers.
*/

.tina-tailwind small {
	font-size: 80%;
}

/**
Prevent 'sub' and 'sup' elements from affecting the line height in all browsers.
*/

.tina-tailwind sub,
.tina-tailwind sup {
	font-size: 75%;
	line-height: 0;
	position: relative;
	vertical-align: baseline;
}

/*
Text-level semantics
====================
*/

.tina-tailwind sub {
	bottom: -0.25em;
}

/*
Grouping content
================
*/

.tina-tailwind sup {
	top: -0.5em;
}

/**
1. Remove text indentation from table contents in Chrome and Safari. (https://bugs.chromium.org/p/chromium/issues/detail?id=999088, https://bugs.webkit.org/show_bug.cgi?id=201297)
2. Correct table border color inheritance in all Chrome and Safari. (https://bugs.chromium.org/p/chromium/issues/detail?id=935729, https://bugs.webkit.org/show_bug.cgi?id=195016)
*/

.tina-tailwind table {
	text-indent: 0; /* 1 */
	border-color: inherit; /* 2 */
}

/**
1. Change the font styles in all browsers.
2. Remove the margin in Firefox and Safari.
*/

.tina-tailwind button,
.tina-tailwind input,
.tina-tailwind optgroup,
.tina-tailwind select,
.tina-tailwind textarea {
	font-family: inherit; /* 1 */
	font-size: 100%; /* 1 */
	line-height: 1.15; /* 1 */
	margin: 0; /* 2 */
}

/**
Remove the inheritance of text transform in Edge and Firefox.
1. Remove the inheritance of text transform in Firefox.
*/

.tina-tailwind button,
.tina-tailwind select { /* 1 */
	text-transform: none;
}

/**
Correct the inability to style clickable types in iOS and Safari.
*/

.tina-tailwind button,
.tina-tailwind [type='button'],
.tina-tailwind [type='reset'],
.tina-tailwind [type='submit'] {
	-webkit-appearance: button;
}

/**
Remove the inner border and padding in Firefox.
*/

.tina-tailwind ::-moz-focus-inner {
	border-style: none;
	padding: 0;
}

/**
Restore the focus styles unset by the previous rule.
*/

.tina-tailwind :-moz-focusring {
	outline: 1px dotted ButtonText;
}

/**
Remove the additional ':invalid' styles in Firefox.
See: https://github.com/mozilla/gecko-dev/blob/2f9eacd9d3d995c937b4251a5557d95d494c9be1/layout/style/res/forms.css#L728-L737
*/

.tina-tailwind :-moz-ui-invalid {
	box-shadow: none;
}

/**
Remove the padding so developers are not caught out when they zero out 'fieldset' elements in all browsers.
*/

.tina-tailwind legend {
	padding: 0;
}

/**
Add the correct vertical alignment in Chrome and Firefox.
*/

.tina-tailwind progress {
	vertical-align: baseline;
}

/**
Correct the cursor style of increment and decrement buttons in Safari.
*/

.tina-tailwind ::-webkit-inner-spin-button,
.tina-tailwind ::-webkit-outer-spin-button {
	height: auto;
}

/**
1. Correct the odd appearance in Chrome and Safari.
2. Correct the outline style in Safari.
*/

.tina-tailwind [type='search'] {
	-webkit-appearance: textfield; /* 1 */
	outline-offset: -2px; /* 2 */
}

/**
Remove the inner padding in Chrome and Safari on macOS.
*/

.tina-tailwind ::-webkit-search-decoration {
	-webkit-appearance: none;
}

/**
1. Correct the inability to style clickable types in iOS and Safari.
2. Change font properties to 'inherit' in Safari.
*/

.tina-tailwind ::-webkit-file-upload-button {
	-webkit-appearance: button; /* 1 */
	font: inherit; /* 2 */
}

/*
Add the correct display in Chrome and Safari.
*/

.tina-tailwind summary {
	display: list-item;
}

/**
 * Removes the default spacing and border for appropriate elements.
 */

.tina-tailwind blockquote,
.tina-tailwind dl,
.tina-tailwind dd,
.tina-tailwind h1,
.tina-tailwind h2,
.tina-tailwind h3,
.tina-tailwind h4,
.tina-tailwind h5,
.tina-tailwind h6,
.tina-tailwind hr,
.tina-tailwind figure,
.tina-tailwind p,
.tina-tailwind pre {
  margin: 0;
}

/**
 * Manually forked from SUIT CSS Base: https://github.com/suitcss/base
 * A thin layer on top of normalize.css that provides a starting point more
 * suitable for web applications.
 */

.tina-tailwind button {
  background-color: transparent;
  background-image: none;
}

/*
Interactive
===========
*/

.tina-tailwind fieldset {
  margin: 0;
  padding: 0;
}

/*
Forms
=====
*/

.tina-tailwind ol,
.tina-tailwind ul {
  list-style: none;
  margin: 0;
  padding: 0;
}

/**
 * 1. Use the user's configured `sans` font-family (with Tailwind's default
 *    sans-serif font stack as a fallback) as a sane default.
 * 2. Use Tailwind's default "normal" line-height so the user isn't forced
 *    to override it to ensure consistency even when using the default theme.
 */

.tina-tailwind html {
  font-family: Inter var, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, "Noto Sans", sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol", "Noto Color Emoji"; /* 1 */
  line-height: 1.5; /* 2 */
}

/**
 * Inherit font-family and line-height from `html` so users can set them as
 * a class directly on the `html` element.
 */

.tina-tailwind body {
  font-family: inherit;
  line-height: inherit;
}

/**
 * 1. Prevent padding and border from affecting element width.
 *
 *    We used to set this in the html element and inherit from
 *    the parent element for everything else. This caused issues
 *    in shadow-dom-enhanced elements like <details> where the content
 *    is wrapped by a div with box-sizing set to `content-box`.
 *
 *    https://github.com/mozdevs/cssremedy/issues/4
 *
 *
 * 2. Allow adding a border to an element by just adding a border-width.
 *
 *    By default, the way the browser specifies that an element should have no
 *    border is by setting it's border-style to `none` in the user-agent
 *    stylesheet.
 *
 *    In order to easily add borders to elements by just setting the `border-width`
 *    property, we change the default border-style for all elements to `solid`, and
 *    use border-width to hide them instead. This way our `border` utilities only
 *    need to set the `border-width` property instead of the entire `border`
 *    shorthand, making our border utilities much more straightforward to compose.
 *
 *    https://github.com/tailwindcss/tailwindcss/pull/116
 */

.tina-tailwind *,
.tina-tailwind ::before,
.tina-tailwind ::after {
  box-sizing: border-box; /* 1 */
  border-width: 0; /* 2 */
  border-style: solid; /* 2 */
  border-color: currentColor; /* 2 */
}

/*
 * Ensure horizontal rules are visible by default
 */

.tina-tailwind hr {
  border-top-width: 1px;
}

/**
 * Undo the `border-style: none` reset that Normalize applies to images so that
 * our `border-{width}` utilities have the expected effect.
 *
 * The Normalize reset is unnecessary for us since we default the border-width
 * to 0 on all elements.
 *
 * https://github.com/tailwindcss/tailwindcss/issues/362
 */

.tina-tailwind img {
  border-style: solid;
}

/**
 * Tailwind custom reset styles
 */

.tina-tailwind textarea {
  resize: vertical;
}

/*
Tabular data
============
*/

.tina-tailwind input::placeholder,
.tina-tailwind textarea::placeholder {
  opacity: 1;
  color: #918c9e;
}

/*
Sections
========
*/

.tina-tailwind button,
.tina-tailwind [role="button"] {
  cursor: pointer;
}

/**
 * Override legacy focus reset from Normalize with modern Firefox focus styles.
 *
 * This is actually an improvement over the new defaults in Firefox in our testing,
 * as it triggers the better focus styles even for links, which still use a dotted
 * outline in Firefox by default.
 */

.tina-tailwind :-moz-focusring {
	outline: auto;
}

/*
Document
========
*/

.tina-tailwind table {
  border-collapse: collapse;
}

/*! modern-normalize v1.1.0 | MIT License | https://github.com/sindresorhus/modern-normalize */

.tina-tailwind h1,
.tina-tailwind h2,
.tina-tailwind h3,
.tina-tailwind h4,
.tina-tailwind h5,
.tina-tailwind h6 {
  font-size: inherit;
  font-weight: inherit;
}

/**
 * Reset links to optimize for opt-in styling instead of
 * opt-out.
 */

.tina-tailwind a {
  color: inherit;
  text-decoration: inherit;
}

/**
 * Reset form element properties that are easy to forget to
 * style explicitly so you don't inadvertently introduce
 * styles that deviate from your design system. These styles
 * supplement a partial reset that is already applied by
 * normalize.css.
 */

.tina-tailwind button,
.tina-tailwind input,
.tina-tailwind optgroup,
.tina-tailwind select,
.tina-tailwind textarea {
  padding: 0;
  line-height: inherit;
  color: inherit;
}

/**
 * Use the configured 'mono' font family for elements that
 * are expected to be rendered with a monospace font, falling
 * back to the system monospace stack if there is no configured
 * 'mono' font family.
 */

.tina-tailwind pre,
.tina-tailwind code,
.tina-tailwind kbd,
.tina-tailwind samp {
  font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace;
}

/**
 * 1. Make replaced elements `display: block` by default as that's
 *    the behavior you want almost all of the time. Inspired by
 *    CSS Remedy, with `svg` added as well.
 *
 *    https://github.com/mozdevs/cssremedy/issues/14
 * 
 * 2. Add `vertical-align: middle` to align replaced elements more
 *    sensibly by default when overriding `display` by adding a
 *    utility like `inline`.
 *
 *    This can trigger a poorly considered linting error in some
 *    tools but is included by design.
 * 
 *    https://github.com/jensimmons/cssremedy/issues/14#issuecomment-634934210
 */

.tina-tailwind img,
.tina-tailwind svg,
.tina-tailwind video,
.tina-tailwind canvas,
.tina-tailwind audio,
.tina-tailwind iframe,
.tina-tailwind embed,
.tina-tailwind object {
  display: block; /* 1 */
  vertical-align: middle; /* 2 */
}

/**
 * Constrain images and videos to the parent width and preserve
 * their intrinsic aspect ratio.
 *
 * https://github.com/mozdevs/cssremedy/issues/14
 */

.tina-tailwind img,
.tina-tailwind video {
  max-width: 100%;
  height: auto;
}

/**
 * Ensure the default browser behavior of the `hidden` attribute.
 */

.tina-tailwind [hidden] {
  display: none;
}

/*! tailwindcss v2.2.19 | MIT License | https://tailwindcss.com
*/

.tina-tailwind *, .tina-tailwind ::before, .tina-tailwind ::after {
  --tw-translate-x: 0;
  --tw-translate-y: 0;
  --tw-rotate: 0;
  --tw-skew-x: 0;
  --tw-skew-y: 0;
  --tw-scale-x: 1;
  --tw-scale-y: 1;
  --tw-transform: translateX(var(--tw-translate-x)) translateY(var(--tw-translate-y)) rotate(var(--tw-rotate)) skewX(var(--tw-skew-x)) skewY(var(--tw-skew-y)) scaleX(var(--tw-scale-x)) scaleY(var(--tw-scale-y));
}

.tina-tailwind *, .tina-tailwind ::before, .tina-tailwind ::after {
  --tw-border-opacity: 1;
  border-color: rgba(225, 221, 236, var(--tw-border-opacity));
}

.tina-tailwind *, .tina-tailwind ::before, .tina-tailwind ::after {
  --tw-ring-offset-shadow: 0 0 #0000;
  --tw-ring-shadow: 0 0 #0000;
  --tw-shadow: 0 0 #0000;
}

.tina-tailwind *, .tina-tailwind ::before, .tina-tailwind ::after {
  --tw-ring-inset: var(--tw-empty,/*!*/ /*!*/);
  --tw-ring-offset-width: 0px;
  --tw-ring-offset-color: #fff;
  --tw-ring-color: rgba(0, 132, 255, 0.5);
  --tw-ring-offset-shadow: 0 0 #0000;
  --tw-ring-shadow: 0 0 #0000;
  --tw-shadow: 0 0 #0000;
}

.tina-tailwind *, .tina-tailwind ::before, .tina-tailwind ::after {
  --tw-blur: var(--tw-empty,/*!*/ /*!*/);
  --tw-brightness: var(--tw-empty,/*!*/ /*!*/);
  --tw-contrast: var(--tw-empty,/*!*/ /*!*/);
  --tw-grayscale: var(--tw-empty,/*!*/ /*!*/);
  --tw-hue-rotate: var(--tw-empty,/*!*/ /*!*/);
  --tw-invert: var(--tw-empty,/*!*/ /*!*/);
  --tw-saturate: var(--tw-empty,/*!*/ /*!*/);
  --tw-sepia: var(--tw-empty,/*!*/ /*!*/);
  --tw-drop-shadow: var(--tw-empty,/*!*/ /*!*/);
  --tw-filter: var(--tw-blur) var(--tw-brightness) var(--tw-contrast) var(--tw-grayscale) var(--tw-hue-rotate) var(--tw-invert) var(--tw-saturate) var(--tw-sepia) var(--tw-drop-shadow);
}

.tina-tailwind [type='text'],.tina-tailwind [type='email'],.tina-tailwind [type='url'],.tina-tailwind [type='password'],.tina-tailwind [type='number'],.tina-tailwind [type='date'],.tina-tailwind [type='datetime-local'],.tina-tailwind [type='month'],.tina-tailwind [type='search'],.tina-tailwind [type='tel'],.tina-tailwind [type='time'],.tina-tailwind [type='week'],.tina-tailwind [multiple],.tina-tailwind textarea,.tina-tailwind select {
  appearance: none;
  background-color: #fff;
  border-color: #716c7f;
  border-width: 1px;
  border-radius: 0px;
  padding-top: 0.5rem;
  padding-right: 0.75rem;
  padding-bottom: 0.5rem;
  padding-left: 0.75rem;
  font-size: 1rem;
  line-height: 1.5rem;
  --tw-shadow: 0 0 #0000;
}

.tina-tailwind [type='text']:focus, .tina-tailwind [type='email']:focus, .tina-tailwind [type='url']:focus, .tina-tailwind [type='password']:focus, .tina-tailwind [type='number']:focus, .tina-tailwind [type='date']:focus, .tina-tailwind [type='datetime-local']:focus, .tina-tailwind [type='month']:focus, .tina-tailwind [type='search']:focus, .tina-tailwind [type='tel']:focus, .tina-tailwind [type='time']:focus, .tina-tailwind [type='week']:focus, .tina-tailwind [multiple]:focus, .tina-tailwind textarea:focus, .tina-tailwind select:focus {
  outline: 2px solid transparent;
  outline-offset: 2px;
  --tw-ring-inset: var(--tw-empty,/*!*/ /*!*/);
  --tw-ring-offset-width: 0px;
  --tw-ring-offset-color: #fff;
  --tw-ring-color: #0574e4;
  --tw-ring-offset-shadow: var(--tw-ring-inset) 0 0 0 var(--tw-ring-offset-width) var(--tw-ring-offset-color);
  --tw-ring-shadow: var(--tw-ring-inset) 0 0 0 calc(1px + var(--tw-ring-offset-width)) var(--tw-ring-color);
  box-shadow: var(--tw-ring-offset-shadow), var(--tw-ring-shadow), var(--tw-shadow);
  border-color: #0574e4;
}

.tina-tailwind input::placeholder,.tina-tailwind textarea::placeholder {
  color: #716c7f;
  opacity: 1;
}

.tina-tailwind ::-webkit-datetime-edit-fields-wrapper {
  padding: 0;
}

.tina-tailwind ::-webkit-date-and-time-value {
  min-height: 1.5em;
}

.tina-tailwind select {
  background-image: url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%23716c7f' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e");
  background-position: right 0.5rem center;
  background-repeat: no-repeat;
  background-size: 1.5em 1.5em;
  padding-right: 2.5rem;
  color-adjust: exact;
}

.tina-tailwind [multiple] {
  background-image: initial;
  background-position: initial;
  background-repeat: unset;
  background-size: initial;
  padding-right: 0.75rem;
  color-adjust: unset;
}

.tina-tailwind [type='checkbox'],.tina-tailwind [type='radio'] {
  appearance: none;
  padding: 0;
  color-adjust: exact;
  display: inline-block;
  vertical-align: middle;
  background-origin: border-box;
  user-select: none;
  flex-shrink: 0;
  height: 1rem;
  width: 1rem;
  color: #0574e4;
  background-color: #fff;
  border-color: #716c7f;
  border-width: 1px;
  --tw-shadow: 0 0 #0000;
}

.tina-tailwind [type='checkbox'] {
  border-radius: 0px;
}

.tina-tailwind [type='radio'] {
  border-radius: 100%;
}

.tina-tailwind [type='checkbox']:focus,.tina-tailwind [type='radio']:focus {
  outline: 2px solid transparent;
  outline-offset: 2px;
  --tw-ring-inset: var(--tw-empty,/*!*/ /*!*/);
  --tw-ring-offset-width: 2px;
  --tw-ring-offset-color: #fff;
  --tw-ring-color: #0574e4;
  --tw-ring-offset-shadow: var(--tw-ring-inset) 0 0 0 var(--tw-ring-offset-width) var(--tw-ring-offset-color);
  --tw-ring-shadow: var(--tw-ring-inset) 0 0 0 calc(2px + var(--tw-ring-offset-width)) var(--tw-ring-color);
  box-shadow: var(--tw-ring-offset-shadow), var(--tw-ring-shadow), var(--tw-shadow);
}

.tina-tailwind [type='checkbox']:checked,.tina-tailwind [type='radio']:checked {
  border-color: transparent;
  background-color: currentColor;
  background-size: 100% 100%;
  background-position: center;
  background-repeat: no-repeat;
}

.tina-tailwind [type='checkbox']:checked {
  background-image: url("data:image/svg+xml,%3csvg viewBox='0 0 16 16' fill='white' xmlns='http://www.w3.org/2000/svg'%3e%3cpath d='M12.207 4.793a1 1 0 010 1.414l-5 5a1 1 0 01-1.414 0l-2-2a1 1 0 011.414-1.414L6.5 9.086l4.293-4.293a1 1 0 011.414 0z'/%3e%3c/svg%3e");
}

.tina-tailwind [type='radio']:checked {
  background-image: url("data:image/svg+xml,%3csvg viewBox='0 0 16 16' fill='white' xmlns='http://www.w3.org/2000/svg'%3e%3ccircle cx='8' cy='8' r='3'/%3e%3c/svg%3e");
}

.tina-tailwind [type='checkbox']:checked:hover,.tina-tailwind [type='checkbox']:checked:focus,.tina-tailwind [type='radio']:checked:hover,.tina-tailwind [type='radio']:checked:focus {
  border-color: transparent;
  background-color: currentColor;
}

.tina-tailwind [type='checkbox']:indeterminate {
  background-image: url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 16 16'%3e%3cpath stroke='white' stroke-linecap='round' stroke-linejoin='round' stroke-width='2' d='M4 8h8'/%3e%3c/svg%3e");
  border-color: transparent;
  background-color: currentColor;
  background-size: 100% 100%;
  background-position: center;
  background-repeat: no-repeat;
}

.tina-tailwind [type='checkbox']:indeterminate:hover,.tina-tailwind [type='checkbox']:indeterminate:focus {
  border-color: transparent;
  background-color: currentColor;
}

.tina-tailwind [type='file'] {
  background: unset;
  border-color: inherit;
  border-width: 0;
  border-radius: 0;
  padding: 0;
  font-size: unset;
  line-height: inherit;
}

.tina-tailwind [type='file']:focus {
  outline: 1px auto -webkit-focus-ring-color;
}

.tina-tailwind .container {
  width: 100%;
}

@media (min-width: 640px) {

  .tina-tailwind .container {
    max-width: 640px;
  }
}

@media (min-width: 768px) {

  .tina-tailwind .container {
    max-width: 768px;
  }
}

@media (min-width: 1024px) {

  .tina-tailwind .container {
    max-width: 1024px;
  }
}

@media (min-width: 1280px) {

  .tina-tailwind .container {
    max-width: 1280px;
  }
}

@media (min-width: 1536px) {

  .tina-tailwind .container {
    max-width: 1536px;
  }
}

.tina-tailwind .prose {
  color: #433e52;
  max-width: 65ch;
}

.tina-tailwind .prose [class~="lead"] {
  color: #565165;
  font-size: 1.25em;
  line-height: 1.6;
  margin-top: 1.2em;
  margin-bottom: 1.2em;
}

.tina-tailwind .prose a {
  color: #252336;
  text-decoration: underline;
  font-weight: 500;
}

.tina-tailwind .prose strong {
  color: #252336;
  font-weight: 600;
}

.tina-tailwind .prose ol[type="A"] {
  --list-counter-style: upper-alpha;
}

.tina-tailwind .prose ol[type="a"] {
  --list-counter-style: lower-alpha;
}

.tina-tailwind .prose ol[type="A" s] {
  --list-counter-style: upper-alpha;
}

.tina-tailwind .prose ol[type="a" s] {
  --list-counter-style: lower-alpha;
}

.tina-tailwind .prose ol[type="I"] {
  --list-counter-style: upper-roman;
}

.tina-tailwind .prose ol[type="i"] {
  --list-counter-style: lower-roman;
}

.tina-tailwind .prose ol[type="I" s] {
  --list-counter-style: upper-roman;
}

.tina-tailwind .prose ol[type="i" s] {
  --list-counter-style: lower-roman;
}

.tina-tailwind .prose ol[type="1"] {
  --list-counter-style: decimal;
}

.tina-tailwind .prose ol > li {
  position: relative;
  padding-left: 1.75em;
}

.tina-tailwind .prose ol > li::before {
  content: counter(list-item, var(--list-counter-style, decimal)) ".";
  position: absolute;
  font-weight: 400;
  color: #716c7f;
  left: 0;
}

.tina-tailwind .prose ul > li {
  position: relative;
  padding-left: 1.75em;
}

.tina-tailwind .prose ul > li::before {
  content: "";
  position: absolute;
  background-color: #b2adbe;
  border-radius: 50%;
  width: 0.375em;
  height: 0.375em;
  top: calc(0.875em - 0.1875em);
  left: 0.25em;
}

.tina-tailwind .prose hr {
  border-color: #E1DDEC;
  border-top-width: 1px;
  margin-top: 3em;
  margin-bottom: 3em;
}

.tina-tailwind .prose blockquote {
  font-weight: 500;
  font-style: italic;
  color: #252336;
  border-left-width: 0.25rem;
  border-left-color: #E1DDEC;
  quotes: "\201C""\201D""\2018""\2019";
  margin-top: 1.6em;
  margin-bottom: 1.6em;
  padding-left: 1em;
}

.tina-tailwind .prose blockquote p:first-of-type::before {
  content: open-quote;
}

.tina-tailwind .prose blockquote p:last-of-type::after {
  content: close-quote;
}

.tina-tailwind .prose h1 {
  color: #252336;
  font-weight: 800;
  font-size: 2.25em;
  margin-top: 0;
  margin-bottom: 0.8888889em;
  line-height: 1.1111111;
}

.tina-tailwind .prose h2 {
  color: #252336;
  font-weight: 700;
  font-size: 1.5em;
  margin-top: 2em;
  margin-bottom: 1em;
  line-height: 1.3333333;
}

.tina-tailwind .prose h3 {
  color: #252336;
  font-weight: 600;
  font-size: 1.25em;
  margin-top: 1.6em;
  margin-bottom: 0.6em;
  line-height: 1.6;
}

.tina-tailwind .prose h4 {
  color: #252336;
  font-weight: 600;
  margin-top: 1.5em;
  margin-bottom: 0.5em;
  line-height: 1.5;
}

.tina-tailwind .prose figure figcaption {
  color: #716c7f;
  font-size: 0.875em;
  line-height: 1.4285714;
  margin-top: 0.8571429em;
}

.tina-tailwind .prose code {
  color: #252336;
  font-weight: 600;
  font-size: 0.875em;
}

.tina-tailwind .prose code::before {
  content: "`";
}

.tina-tailwind .prose code::after {
  content: "`";
}

.tina-tailwind .prose a code {
  color: #252336;
}

.tina-tailwind .prose pre {
  color: #E1DDEC;
  background-color: #363145;
  overflow-x: auto;
  font-size: 0.875em;
  line-height: 1.7142857;
  margin-top: 1.7142857em;
  margin-bottom: 1.7142857em;
  border-radius: 0.375rem;
  padding-top: 0.8571429em;
  padding-right: 1.1428571em;
  padding-bottom: 0.8571429em;
  padding-left: 1.1428571em;
}

.tina-tailwind .prose pre code {
  background-color: transparent;
  border-width: 0;
  border-radius: 0;
  padding: 0;
  font-weight: 400;
  color: inherit;
  font-size: inherit;
  font-family: inherit;
  line-height: inherit;
}

.tina-tailwind .prose pre code::before {
  content: none;
}

.tina-tailwind .prose pre code::after {
  content: none;
}

.tina-tailwind .prose table {
  width: 100%;
  table-layout: auto;
  text-align: left;
  margin-top: 2em;
  margin-bottom: 2em;
  font-size: 0.875em;
  line-height: 1.7142857;
}

.tina-tailwind .prose thead {
  color: #252336;
  font-weight: 600;
  border-bottom-width: 1px;
  border-bottom-color: #b2adbe;
}

.tina-tailwind .prose thead th {
  vertical-align: bottom;
  padding-right: 0.5714286em;
  padding-bottom: 0.5714286em;
  padding-left: 0.5714286em;
}

.tina-tailwind .prose tbody tr {
  border-bottom-width: 1px;
  border-bottom-color: #E1DDEC;
}

.tina-tailwind .prose tbody tr:last-child {
  border-bottom-width: 0;
}

.tina-tailwind .prose tbody td {
  vertical-align: top;
  padding-top: 0.5714286em;
  padding-right: 0.5714286em;
  padding-bottom: 0.5714286em;
  padding-left: 0.5714286em;
}

.tina-tailwind .prose {
  font-size: 1rem;
  line-height: 1.75;
}

.tina-tailwind .prose p {
  margin-top: 1.25em;
  margin-bottom: 1.25em;
}

.tina-tailwind .prose img {
  margin-top: 2em;
  margin-bottom: 2em;
}

.tina-tailwind .prose video {
  margin-top: 2em;
  margin-bottom: 2em;
}

.tina-tailwind .prose figure {
  margin-top: 2em;
  margin-bottom: 2em;
}

.tina-tailwind .prose figure > * {
  margin-top: 0;
  margin-bottom: 0;
}

.tina-tailwind .prose h2 code {
  font-size: 0.875em;
}

.tina-tailwind .prose h3 code {
  font-size: 0.9em;
}

.tina-tailwind .prose ol {
  margin-top: 1.25em;
  margin-bottom: 1.25em;
}

.tina-tailwind .prose ul {
  margin-top: 1.25em;
  margin-bottom: 1.25em;
}

.tina-tailwind .prose li {
  margin-top: 0.5em;
  margin-bottom: 0.5em;
}

.tina-tailwind .prose > ul > li p {
  margin-top: 0.75em;
  margin-bottom: 0.75em;
}

.tina-tailwind .prose > ul > li > *:first-child {
  margin-top: 1.25em;
}

.tina-tailwind .prose > ul > li > *:last-child {
  margin-bottom: 1.25em;
}

.tina-tailwind .prose > ol > li > *:first-child {
  margin-top: 1.25em;
}

.tina-tailwind .prose > ol > li > *:last-child {
  margin-bottom: 1.25em;
}

.tina-tailwind .prose ul ul, .tina-tailwind .prose ul ol, .tina-tailwind .prose ol ul, .tina-tailwind .prose ol ol {
  margin-top: 0.75em;
  margin-bottom: 0.75em;
}

.tina-tailwind .prose hr + * {
  margin-top: 0;
}

.tina-tailwind .prose h2 + * {
  margin-top: 0;
}

.tina-tailwind .prose h3 + * {
  margin-top: 0;
}

.tina-tailwind .prose h4 + * {
  margin-top: 0;
}

.tina-tailwind .prose thead th:first-child {
  padding-left: 0;
}

.tina-tailwind .prose thead th:last-child {
  padding-right: 0;
}

.tina-tailwind .prose tbody td:first-child {
  padding-left: 0;
}

.tina-tailwind .prose tbody td:last-child {
  padding-right: 0;
}

.tina-tailwind .prose > :first-child {
  margin-top: 0;
}

.tina-tailwind .prose > :last-child {
  margin-bottom: 0;
}

.tina-tailwind .sr-only {
  position: absolute !important;
  width: 1px !important;
  height: 1px !important;
  padding: 0 !important;
  margin: -1px !important;
  overflow: hidden !important;
  clip: rect(0, 0, 0, 0) !important;
  white-space: nowrap !important;
  border-width: 0 !important;
}

.tina-tailwind .pointer-events-none {
  pointer-events: none !important;
}

.tina-tailwind .pointer-events-auto {
  pointer-events: auto !important;
}

.tina-tailwind .visible {
  visibility: visible !important;
}

.tina-tailwind .\!visible {
  visibility: visible !important;
}

.tina-tailwind .static {
  position: static !important;
}

.tina-tailwind .fixed {
  position: fixed !important;
}

.tina-tailwind .absolute {
  position: absolute !important;
}

.tina-tailwind .relative {
  position: relative !important;
}

.tina-tailwind .sticky {
  position: sticky !important;
}

.tina-tailwind .inset-0 {
  top: 0px !important;
  right: 0px !important;
  bottom: 0px !important;
  left: 0px !important;
}

.tina-tailwind .top-1\/2 {
  top: 50% !important;
}

.tina-tailwind .right-1 {
  right: 4px !important;
}

.tina-tailwind .left-1\/2 {
  left: 50% !important;
}

.tina-tailwind .bottom-12 {
  bottom: 48px !important;
}

.tina-tailwind .right-0 {
  right: 0px !important;
}

.tina-tailwind .top-0 {
  top: 0px !important;
}

.tina-tailwind .left-0 {
  left: 0px !important;
}

.tina-tailwind .top-4 {
  top: 16px !important;
}

.tina-tailwind .left-2 {
  left: 8px !important;
}

.tina-tailwind .-top-6 {
  top: -24px !important;
}

.tina-tailwind .z-10 {
  z-index: 10 !important;
}

.tina-tailwind .z-40 {
  z-index: 40 !important;
}

.tina-tailwind .z-overlay {
  z-index: 10600 !important;
}

.tina-tailwind .z-100 {
  z-index: 100 !important;
}

.tina-tailwind .z-chrome {
  z-index: 10200 !important;
}

.tina-tailwind .z-menu {
  z-index: 9800 !important;
}

.tina-tailwind .z-base {
  z-index: 9000 !important;
}

.tina-tailwind .z-50 {
  z-index: 50 !important;
}

.tina-tailwind .z-20 {
  z-index: 20 !important;
}

.tina-tailwind .m-0 {
  margin: 0px !important;
}

.tina-tailwind .mx-auto {
  margin-left: auto !important;
  margin-right: auto !important;
}

.tina-tailwind .-mx-0\.5 {
  margin-left: -2px !important;
  margin-right: -2px !important;
}

.tina-tailwind .-mx-0 {
  margin-left: 0px !important;
  margin-right: 0px !important;
}

.tina-tailwind .mx-0\.5 {
  margin-left: 2px !important;
  margin-right: 2px !important;
}

.tina-tailwind .mx-0 {
  margin-left: 0px !important;
  margin-right: 0px !important;
}

.tina-tailwind .my-0 {
  margin-top: 0px !important;
  margin-bottom: 0px !important;
}

.tina-tailwind .ml-2 {
  margin-left: 8px !important;
}

.tina-tailwind .mr-2 {
  margin-right: 8px !important;
}

.tina-tailwind .mt-2 {
  margin-top: 8px !important;
}

.tina-tailwind .mb-2 {
  margin-bottom: 8px !important;
}

.tina-tailwind .mb-5 {
  margin-bottom: 20px !important;
}

.tina-tailwind .-mt-px {
  margin-top: -1px !important;
}

.tina-tailwind .-mt-0\.5 {
  margin-top: -2px !important;
}

.tina-tailwind .-mt-0 {
  margin-top: 0px !important;
}

.tina-tailwind .mb-6 {
  margin-bottom: 24px !important;
}

.tina-tailwind .mb-4 {
  margin-bottom: 16px !important;
}

.tina-tailwind .mt-12 {
  margin-top: 48px !important;
}

.tina-tailwind .-mr-1 {
  margin-right: -4px !important;
}

.tina-tailwind .-ml-px {
  margin-left: -1px !important;
}

.tina-tailwind .block {
  display: block !important;
}

.tina-tailwind .inline-block {
  display: inline-block !important;
}

.tina-tailwind .inline {
  display: inline !important;
}

.tina-tailwind .flex {
  display: flex !important;
}

.tina-tailwind .inline-flex {
  display: inline-flex !important;
}

.tina-tailwind .table {
  display: table !important;
}

.tina-tailwind .grid {
  display: grid !important;
}

.tina-tailwind .contents {
  display: contents !important;
}

.tina-tailwind .hidden {
  display: none !important;
}

.tina-tailwind .h-auto {
  height: auto !important;
}

.tina-tailwind .h-full {
  height: 100% !important;
}

.tina-tailwind .h-16 {
  height: 64px !important;
}

.tina-tailwind .h-screen {
  height: 100vh !important;
}

.tina-tailwind .h-3 {
  height: 12px !important;
}

.tina-tailwind .h-8 {
  height: 32px !important;
}

.tina-tailwind .h-10 {
  height: 40px !important;
}

.tina-tailwind .h-6 {
  height: 24px !important;
}

.tina-tailwind .h-5 {
  height: 20px !important;
}

.tina-tailwind .h-14 {
  height: 56px !important;
}

.tina-tailwind .h-32 {
  height: 128px !important;
}

.tina-tailwind .h-4\/6 {
  height: 66.666667% !important;
}

.tina-tailwind .h-11 {
  height: 44px !important;
}

.tina-tailwind .max-h-full {
  max-height: 100% !important;
}

.tina-tailwind .w-8 {
  width: 32px !important;
}

.tina-tailwind .w-5\/6 {
  width: 83.333333% !important;
}

.tina-tailwind .w-full {
  width: 100% !important;
}

.tina-tailwind .w-3 {
  width: 12px !important;
}

.tina-tailwind .w-6 {
  width: 24px !important;
}

.tina-tailwind .w-auto {
  width: auto !important;
}

.tina-tailwind .w-7 {
  width: 28px !important;
}

.tina-tailwind .w-5 {
  width: 20px !important;
}

.tina-tailwind .w-4 {
  width: 16px !important;
}

.tina-tailwind .w-screen {
  width: 100vw !important;
}

.tina-tailwind .w-px {
  width: 1px !important;
}

.tina-tailwind .w-14 {
  width: 56px !important;
}

.tina-tailwind .w-96 {
  width: 384px !important;
}

.tina-tailwind .w-56 {
  width: 224px !important;
}

.tina-tailwind .w-32 {
  width: 128px !important;
}

.tina-tailwind .max-w-form {
  max-width: 900px !important;
}

.tina-tailwind .max-w-prose {
  max-width: 65ch !important;
}

.tina-tailwind .max-w-screen-xl {
  max-width: 1280px !important;
}

.tina-tailwind .max-w-full {
  max-width: 100% !important;
}

.tina-tailwind .flex-1 {
  flex: 1 1 0% !important;
}

.tina-tailwind .flex-none {
  flex: none !important;
}

.tina-tailwind .flex-shrink {
  flex-shrink: 1 !important;
}

.tina-tailwind .flex-shrink-0 {
  flex-shrink: 0 !important;
}

.tina-tailwind .flex-grow {
  flex-grow: 1 !important;
}

.tina-tailwind .origin-top-right {
  transform-origin: top right !important;
}

.tina-tailwind .origin-top-left {
  transform-origin: top left !important;
}

.tina-tailwind .translate-x-1\/2 {
  --tw-translate-x: 50% !important;
  transform: var(--tw-transform) !important;
}

.tina-tailwind .-translate-y-1\/2 {
  --tw-translate-y: -50% !important;
  transform: var(--tw-transform) !important;
}

.tina-tailwind .-translate-x-1\/2 {
  --tw-translate-x: -50% !important;
  transform: var(--tw-transform) !important;
}

.tina-tailwind .translate-x-full {
  --tw-translate-x: 100% !important;
  transform: var(--tw-transform) !important;
}

.tina-tailwind .-translate-x-full {
  --tw-translate-x: -100% !important;
  transform: var(--tw-transform) !important;
}

.tina-tailwind .scale-110 {
  --tw-scale-x: 1.1 !important;
  --tw-scale-y: 1.1 !important;
  transform: var(--tw-transform) !important;
}

.tina-tailwind .scale-95 {
  --tw-scale-x: .95 !important;
  --tw-scale-y: .95 !important;
  transform: var(--tw-transform) !important;
}

.tina-tailwind .scale-100 {
  --tw-scale-x: 1 !important;
  --tw-scale-y: 1 !important;
  transform: var(--tw-transform) !important;
}

.tina-tailwind .transform {
  transform: var(--tw-transform) !important;
}

.tina-tailwind .cursor-not-allowed {
  cursor: not-allowed !important;
}

.tina-tailwind .cursor-wait {
  cursor: wait !important;
}

.tina-tailwind .cursor-pointer {
  cursor: pointer !important;
}

.tina-tailwind .select-none {
  user-select: none !important;
}

.tina-tailwind .resize-y {
  resize: vertical !important;
}

.tina-tailwind .resize {
  resize: both !important;
}

.tina-tailwind .auto-cols-fr {
  grid-auto-columns: minmax(0, 1fr) !important;
}

.tina-tailwind .grid-flow-col {
  grid-auto-flow: column !important;
}

.tina-tailwind .flex-col {
  flex-direction: column !important;
}

.tina-tailwind .flex-wrap {
  flex-wrap: wrap !important;
}

.tina-tailwind .flex-nowrap {
  flex-wrap: nowrap !important;
}

.tina-tailwind .items-center {
  align-items: center !important;
}

.tina-tailwind .justify-end {
  justify-content: flex-end !important;
}

.tina-tailwind .justify-center {
  justify-content: center !important;
}

.tina-tailwind .justify-between {
  justify-content: space-between !important;
}

.tina-tailwind .gap-2 {
  gap: 8px !important;
}

.tina-tailwind .gap-4 {
  gap: 16px !important;
}

.tina-tailwind .gap-3 {
  gap: 12px !important;
}

.tina-tailwind .gap-x-6 {
  column-gap: 24px !important;
}

.tina-tailwind .gap-y-3 {
  row-gap: 12px !important;
}

.tina-tailwind .divide-y > :not([hidden]) ~ :not([hidden]) {
  --tw-divide-y-reverse: 0 !important;
  border-top-width: calc(1px * calc(1 - var(--tw-divide-y-reverse))) !important;
  border-bottom-width: calc(1px * var(--tw-divide-y-reverse)) !important;
}

.tina-tailwind .divide-gray-100 > :not([hidden]) ~ :not([hidden]) {
  --tw-divide-opacity: 1 !important;
  border-color: rgba(237, 236, 243, var(--tw-divide-opacity)) !important;
}

.tina-tailwind .overflow-hidden {
  overflow: hidden !important;
}

.tina-tailwind .overflow-y-auto {
  overflow-y: auto !important;
}

.tina-tailwind .overflow-x-hidden {
  overflow-x: hidden !important;
}

.tina-tailwind .overflow-y-visible {
  overflow-y: visible !important;
}

.tina-tailwind .overflow-x-scroll {
  overflow-x: scroll !important;
}

.tina-tailwind .overflow-ellipsis {
  text-overflow: ellipsis !important;
}

.tina-tailwind .whitespace-nowrap {
  white-space: nowrap !important;
}

.tina-tailwind .rounded-lg {
  border-radius: 8px !important;
}

.tina-tailwind .rounded-full {
  border-radius: 9999px !important;
}

.tina-tailwind .rounded {
  border-radius: 4px !important;
}

.tina-tailwind .rounded-md {
  border-radius: 6px !important;
}

.tina-tailwind .rounded-l-full {
  border-top-left-radius: 9999px !important;
  border-bottom-left-radius: 9999px !important;
}

.tina-tailwind .rounded-r-full {
  border-top-right-radius: 9999px !important;
  border-bottom-right-radius: 9999px !important;
}

.tina-tailwind .rounded-r-md {
  border-top-right-radius: 6px !important;
  border-bottom-right-radius: 6px !important;
}

.tina-tailwind .rounded-l-md {
  border-top-left-radius: 6px !important;
  border-bottom-left-radius: 6px !important;
}

.tina-tailwind .border {
  border-width: 1px !important;
}

.tina-tailwind .border-2 {
  border-width: 2px !important;
}

.tina-tailwind .border-0 {
  border-width: 0 !important;
}

.tina-tailwind .border-b {
  border-bottom-width: 1px !important;
}

.tina-tailwind .border-t {
  border-top-width: 1px !important;
}

.tina-tailwind .border-b-0 {
  border-bottom-width: 0 !important;
}

.tina-tailwind .border-gray-100 {
  --tw-border-opacity: 1 !important;
  border-color: rgba(237, 236, 243, var(--tw-border-opacity)) !important;
}

.tina-tailwind .border-blue-500 {
  --tw-border-opacity: 1 !important;
  border-color: rgba(0, 132, 255, var(--tw-border-opacity)) !important;
}

.tina-tailwind .border-yellow-500 {
  --tw-border-opacity: 1 !important;
  border-color: rgba(245, 158, 11, var(--tw-border-opacity)) !important;
}

.tina-tailwind .border-green-400 {
  --tw-border-opacity: 1 !important;
  border-color: rgba(52, 211, 153, var(--tw-border-opacity)) !important;
}

.tina-tailwind .border-gray-50 {
  --tw-border-opacity: 1 !important;
  border-color: rgba(246, 246, 249, var(--tw-border-opacity)) !important;
}

.tina-tailwind .border-transparent {
  border-color: transparent !important;
}

.tina-tailwind .border-gray-200 {
  --tw-border-opacity: 1 !important;
  border-color: rgba(225, 221, 236, var(--tw-border-opacity)) !important;
}

.tina-tailwind .border-gray-300 {
  --tw-border-opacity: 1 !important;
  border-color: rgba(178, 173, 190, var(--tw-border-opacity)) !important;
}

.tina-tailwind .bg-white {
  --tw-bg-opacity: 1 !important;
  background-color: rgba(255, 255, 255, var(--tw-bg-opacity)) !important;
}

.tina-tailwind .bg-yellow-400 {
  --tw-bg-opacity: 1 !important;
  background-color: rgba(251, 191, 36, var(--tw-bg-opacity)) !important;
}

.tina-tailwind .bg-green-300 {
  --tw-bg-opacity: 1 !important;
  background-color: rgba(110, 231, 183, var(--tw-bg-opacity)) !important;
}

.tina-tailwind .bg-gray-50 {
  --tw-bg-opacity: 1 !important;
  background-color: rgba(246, 246, 249, var(--tw-bg-opacity)) !important;
}

.tina-tailwind .bg-blue-500 {
  --tw-bg-opacity: 1 !important;
  background-color: rgba(0, 132, 255, var(--tw-bg-opacity)) !important;
}

.tina-tailwind .bg-gray-100 {
  --tw-bg-opacity: 1 !important;
  background-color: rgba(237, 236, 243, var(--tw-bg-opacity)) !important;
}

.tina-tailwind .bg-transparent {
  background-color: transparent !important;
}

.tina-tailwind .bg-gray-800 {
  --tw-bg-opacity: 1 !important;
  background-color: rgba(54, 49, 69, var(--tw-bg-opacity)) !important;
}

.tina-tailwind .bg-gray-300 {
  --tw-bg-opacity: 1 !important;
  background-color: rgba(178, 173, 190, var(--tw-bg-opacity)) !important;
}

.tina-tailwind .bg-gray-200 {
  --tw-bg-opacity: 1 !important;
  background-color: rgba(225, 221, 236, var(--tw-bg-opacity)) !important;
}

.tina-tailwind .bg-opacity-80 {
  --tw-bg-opacity: .8 !important;
}

.tina-tailwind .p-5 {
  padding: 20px !important;
}

.tina-tailwind .p-2 {
  padding: 8px !important;
}

.tina-tailwind .p-4 {
  padding: 16px !important;
}

.tina-tailwind .py-3 {
  padding-top: 12px !important;
  padding-bottom: 12px !important;
}

.tina-tailwind .px-5 {
  padding-left: 20px !important;
  padding-right: 20px !important;
}

.tina-tailwind .px-6 {
  padding-left: 24px !important;
  padding-right: 24px !important;
}

.tina-tailwind .py-4 {
  padding-top: 16px !important;
  padding-bottom: 16px !important;
}

.tina-tailwind .px-4 {
  padding-left: 16px !important;
  padding-right: 16px !important;
}

.tina-tailwind .px-3 {
  padding-left: 12px !important;
  padding-right: 12px !important;
}

.tina-tailwind .py-2 {
  padding-top: 8px !important;
  padding-bottom: 8px !important;
}

.tina-tailwind .px-2 {
  padding-left: 8px !important;
  padding-right: 8px !important;
}

.tina-tailwind .py-1\.5 {
  padding-top: 6px !important;
  padding-bottom: 6px !important;
}

.tina-tailwind .py-1 {
  padding-top: 4px !important;
  padding-bottom: 4px !important;
}

.tina-tailwind .py-0\.5 {
  padding-top: 2px !important;
  padding-bottom: 2px !important;
}

.tina-tailwind .py-0 {
  padding-top: 0px !important;
  padding-bottom: 0px !important;
}

.tina-tailwind .px-1 {
  padding-left: 4px !important;
  padding-right: 4px !important;
}

.tina-tailwind .pt-8 {
  padding-top: 32px !important;
}

.tina-tailwind .pb-2 {
  padding-bottom: 8px !important;
}

.tina-tailwind .pt-1 {
  padding-top: 4px !important;
}

.tina-tailwind .pt-12 {
  padding-top: 48px !important;
}

.tina-tailwind .pb-8 {
  padding-bottom: 32px !important;
}

.tina-tailwind .text-left {
  text-align: left !important;
}

.tina-tailwind .text-center {
  text-align: center !important;
}

.tina-tailwind .text-lg {
  font-size: 18px !important;
  line-height: 1.55 !important;
}

.tina-tailwind .text-sm {
  font-size: 14px !important;
  line-height: 1.43 !important;
}

.tina-tailwind .text-xs {
  font-size: 13px !important;
  line-height: 1.33 !important;
}

.tina-tailwind .text-base {
  font-size: 16px !important;
  line-height: 1.5 !important;
}

.tina-tailwind .text-xl {
  font-size: 20px !important;
  line-height: 1.4 !important;
}

.tina-tailwind .font-bold {
  font-weight: 700 !important;
}

.tina-tailwind .font-medium {
  font-weight: 500 !important;
}

.tina-tailwind .uppercase {
  text-transform: uppercase !important;
}

.tina-tailwind .italic {
  font-style: italic !important;
}

.tina-tailwind .leading-tight {
  line-height: 1.25 !important;
}

.tina-tailwind .leading-none {
  line-height: 1 !important;
}

.tina-tailwind .tracking-wide {
  letter-spacing: 0.025em !important;
}

.tina-tailwind .text-gray-700 {
  --tw-text-opacity: 1 !important;
  color: rgba(67, 62, 82, var(--tw-text-opacity)) !important;
}

.tina-tailwind .text-gray-500 {
  --tw-text-opacity: 1 !important;
  color: rgba(113, 108, 127, var(--tw-text-opacity)) !important;
}

.tina-tailwind .text-white {
  --tw-text-opacity: 1 !important;
  color: rgba(255, 255, 255, var(--tw-text-opacity)) !important;
}

.tina-tailwind .text-gray-600 {
  --tw-text-opacity: 1 !important;
  color: rgba(86, 81, 101, var(--tw-text-opacity)) !important;
}

.tina-tailwind .text-gray-50 {
  --tw-text-opacity: 1 !important;
  color: rgba(246, 246, 249, var(--tw-text-opacity)) !important;
}

.tina-tailwind .text-blue-500 {
  --tw-text-opacity: 1 !important;
  color: rgba(0, 132, 255, var(--tw-text-opacity)) !important;
}

.tina-tailwind .text-gray-900 {
  --tw-text-opacity: 1 !important;
  color: rgba(37, 35, 54, var(--tw-text-opacity)) !important;
}

.tina-tailwind .opacity-80 {
  opacity: .8 !important;
}

.tina-tailwind .opacity-30 {
  opacity: .3 !important;
}

.tina-tailwind .opacity-70 {
  opacity: .7 !important;
}

.tina-tailwind .opacity-100 {
  opacity: 1 !important;
}

.tina-tailwind .opacity-0 {
  opacity: 0 !important;
}

.tina-tailwind .opacity-50 {
  opacity: .5 !important;
}

.tina-tailwind .shadow-sm {
  --tw-shadow: 0 1px 2px 0 rgba(0, 0, 0, 0.05) !important;
  box-shadow: var(--tw-ring-offset-shadow, 0 0 #0000), var(--tw-ring-shadow, 0 0 #0000), var(--tw-shadow) !important;
}

.tina-tailwind .shadow {
  --tw-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.1), 0 1px 2px 0 rgba(0, 0, 0, 0.06) !important;
  box-shadow: var(--tw-ring-offset-shadow, 0 0 #0000), var(--tw-ring-shadow, 0 0 #0000), var(--tw-shadow) !important;
}

.tina-tailwind .shadow-inner {
  --tw-shadow: inset 0 2px 4px 0 rgba(0, 0, 0, 0.06) !important;
  box-shadow: var(--tw-ring-offset-shadow, 0 0 #0000), var(--tw-ring-shadow, 0 0 #0000), var(--tw-shadow) !important;
}

.tina-tailwind .shadow-2xl {
  --tw-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25) !important;
  box-shadow: var(--tw-ring-offset-shadow, 0 0 #0000), var(--tw-ring-shadow, 0 0 #0000), var(--tw-shadow) !important;
}

.tina-tailwind .shadow-outline {
  --tw-shadow: 0 0 0 3px rgba(66, 153, 225, 0.5) !important;
  box-shadow: var(--tw-ring-offset-shadow, 0 0 #0000), var(--tw-ring-shadow, 0 0 #0000), var(--tw-shadow) !important;
}

.tina-tailwind .shadow-lg {
  --tw-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05) !important;
  box-shadow: var(--tw-ring-offset-shadow, 0 0 #0000), var(--tw-ring-shadow, 0 0 #0000), var(--tw-shadow) !important;
}

.tina-tailwind .outline-none {
  outline: 2px solid transparent !important;
  outline-offset: 2px !important;
}

.tina-tailwind .ring-2 {
  --tw-ring-offset-shadow: var(--tw-ring-inset) 0 0 0 var(--tw-ring-offset-width) var(--tw-ring-offset-color) !important;
  --tw-ring-shadow: var(--tw-ring-inset) 0 0 0 calc(2px + var(--tw-ring-offset-width)) var(--tw-ring-color) !important;
  box-shadow: var(--tw-ring-offset-shadow), var(--tw-ring-shadow), var(--tw-shadow, 0 0 #0000) !important;
}

.tina-tailwind .ring-1 {
  --tw-ring-offset-shadow: var(--tw-ring-inset) 0 0 0 var(--tw-ring-offset-width) var(--tw-ring-offset-color) !important;
  --tw-ring-shadow: var(--tw-ring-inset) 0 0 0 calc(1px + var(--tw-ring-offset-width)) var(--tw-ring-color) !important;
  box-shadow: var(--tw-ring-offset-shadow), var(--tw-ring-shadow), var(--tw-shadow, 0 0 #0000) !important;
}

.tina-tailwind .ring-inset {
  --tw-ring-inset: inset !important;
}

.tina-tailwind .ring-blue-500 {
  --tw-ring-opacity: 1 !important;
  --tw-ring-color: rgba(0, 132, 255, var(--tw-ring-opacity)) !important;
}

.tina-tailwind .ring-black {
  --tw-ring-opacity: 1 !important;
  --tw-ring-color: rgba(0, 0, 0, var(--tw-ring-opacity)) !important;
}

.tina-tailwind .ring-opacity-5 {
  --tw-ring-opacity: .05 !important;
}

.tina-tailwind .drop-shadow {
  --tw-drop-shadow: drop-shadow(0 1px 2px rgba(0, 0, 0, 0.1)) drop-shadow(0 1px 1px rgba(0, 0, 0, 0.06)) !important;
  filter: var(--tw-filter) !important;
}

.tina-tailwind .grayscale {
  --tw-grayscale: grayscale(100%) !important;
  filter: var(--tw-filter) !important;
}

.tina-tailwind .filter {
  filter: var(--tw-filter) !important;
}

.tina-tailwind .transition {
  transition-property: background-color, border-color, color, fill, stroke, opacity, box-shadow, transform, filter, backdrop-filter !important;
  transition-timing-function: cubic-bezier(0.4, 0, 0.2, 1) !important;
  transition-duration: 150ms !important;
}

.tina-tailwind .transition-all {
  transition-property: all !important;
  transition-timing-function: cubic-bezier(0.4, 0, 0.2, 1) !important;
  transition-duration: 150ms !important;
}

.tina-tailwind .transition-opacity {
  transition-property: opacity !important;
  transition-timing-function: cubic-bezier(0.4, 0, 0.2, 1) !important;
  transition-duration: 150ms !important;
}

.tina-tailwind .transition-transform {
  transition-property: transform !important;
  transition-timing-function: cubic-bezier(0.4, 0, 0.2, 1) !important;
  transition-duration: 150ms !important;
}

.tina-tailwind .transition-none {
  transition-property: none !important;
}

.tina-tailwind .duration-150 {
  transition-duration: 150ms !important;
}

.tina-tailwind .duration-300 {
  transition-duration: 300ms !important;
}

.tina-tailwind .duration-100 {
  transition-duration: 100ms !important;
}

.tina-tailwind .duration-75 {
  transition-duration: 75ms !important;
}

.tina-tailwind .ease-out {
  transition-timing-function: cubic-bezier(0, 0, 0.2, 1) !important;
}

.tina-tailwind .ease-in {
  transition-timing-function: cubic-bezier(0.4, 0, 1, 1) !important;
}

.tina-tailwind .icon-parent svg {
        fill: currentColor !important;
      }

.tina-tailwind [data-slate-editor='true'] {
  color: #433e52;
  max-width: 65ch;
}

.tina-tailwind [data-slate-editor='true'] [class~="lead"] {
  color: #565165;
  font-size: 1.25em;
  line-height: 1.6;
  margin-top: 1.2em;
  margin-bottom: 1.2em;
}

.tina-tailwind [data-slate-editor='true'] a {
  color: #252336;
  text-decoration: underline;
  font-weight: 500;
}

.tina-tailwind [data-slate-editor='true'] strong {
  color: #252336;
  font-weight: 600;
}

.tina-tailwind [data-slate-editor='true'] ol[type="A"] {
  --list-counter-style: upper-alpha;
}

.tina-tailwind [data-slate-editor='true'] ol[type="a"] {
  --list-counter-style: lower-alpha;
}

.tina-tailwind [data-slate-editor='true'] ol[type="A" s] {
  --list-counter-style: upper-alpha;
}

.tina-tailwind [data-slate-editor='true'] ol[type="a" s] {
  --list-counter-style: lower-alpha;
}

.tina-tailwind [data-slate-editor='true'] ol[type="I"] {
  --list-counter-style: upper-roman;
}

.tina-tailwind [data-slate-editor='true'] ol[type="i"] {
  --list-counter-style: lower-roman;
}

.tina-tailwind [data-slate-editor='true'] ol[type="I" s] {
  --list-counter-style: upper-roman;
}

.tina-tailwind [data-slate-editor='true'] ol[type="i" s] {
  --list-counter-style: lower-roman;
}

.tina-tailwind [data-slate-editor='true'] ol[type="1"] {
  --list-counter-style: decimal;
}

.tina-tailwind [data-slate-editor='true'] ol > li {
  position: relative;
  padding-left: 1.75em;
}

.tina-tailwind [data-slate-editor='true'] ol > li::before {
  content: counter(list-item, var(--list-counter-style, decimal)) ".";
  position: absolute;
  font-weight: 400;
  color: #716c7f;
  left: 0;
}

.tina-tailwind [data-slate-editor='true'] ul > li {
  position: relative;
  padding-left: 1.75em;
}

.tina-tailwind [data-slate-editor='true'] ul > li::before {
  content: "";
  position: absolute;
  background-color: #b2adbe;
  border-radius: 50%;
  width: 0.375em;
  height: 0.375em;
  top: calc(0.875em - 0.1875em);
  left: 0.25em;
}

.tina-tailwind [data-slate-editor='true'] hr {
  border-color: #E1DDEC;
  border-top-width: 1px;
  margin-top: 3em;
  margin-bottom: 3em;
}

.tina-tailwind [data-slate-editor='true'] blockquote {
  font-weight: 500;
  font-style: italic;
  color: #252336;
  border-left-width: 0.25rem;
  border-left-color: #E1DDEC;
  quotes: "\201C""\201D""\2018""\2019";
  margin-top: 1.6em;
  margin-bottom: 1.6em;
  padding-left: 1em;
}

.tina-tailwind [data-slate-editor='true'] blockquote p:first-of-type::before {
  content: open-quote;
}

.tina-tailwind [data-slate-editor='true'] blockquote p:last-of-type::after {
  content: close-quote;
}

.tina-tailwind [data-slate-editor='true'] h1 {
  color: #252336;
  font-weight: 800;
  font-size: 2.25em;
  margin-top: 0;
  margin-bottom: 0.8888889em;
  line-height: 1.1111111;
}

.tina-tailwind [data-slate-editor='true'] h2 {
  color: #252336;
  font-weight: 700;
  font-size: 1.5em;
  margin-top: 2em;
  margin-bottom: 1em;
  line-height: 1.3333333;
}

.tina-tailwind [data-slate-editor='true'] h3 {
  color: #252336;
  font-weight: 600;
  font-size: 1.25em;
  margin-top: 1.6em;
  margin-bottom: 0.6em;
  line-height: 1.6;
}

.tina-tailwind [data-slate-editor='true'] h4 {
  color: #252336;
  font-weight: 600;
  margin-top: 1.5em;
  margin-bottom: 0.5em;
  line-height: 1.5;
}

.tina-tailwind [data-slate-editor='true'] figure figcaption {
  color: #716c7f;
  font-size: 0.875em;
  line-height: 1.4285714;
  margin-top: 0.8571429em;
}

.tina-tailwind [data-slate-editor='true'] code {
  color: #252336;
  font-weight: 600;
  font-size: 0.875em;
}

.tina-tailwind [data-slate-editor='true'] code::before {
  content: "`";
}

.tina-tailwind [data-slate-editor='true'] code::after {
  content: "`";
}

.tina-tailwind [data-slate-editor='true'] a code {
  color: #252336;
}

.tina-tailwind [data-slate-editor='true'] pre {
  color: #E1DDEC;
  background-color: #363145;
  overflow-x: auto;
  font-size: 0.875em;
  line-height: 1.7142857;
  margin-top: 1.7142857em;
  margin-bottom: 1.7142857em;
  border-radius: 0.375rem;
  padding-top: 0.8571429em;
  padding-right: 1.1428571em;
  padding-bottom: 0.8571429em;
  padding-left: 1.1428571em;
}

.tina-tailwind [data-slate-editor='true'] pre code {
  background-color: transparent;
  border-width: 0;
  border-radius: 0;
  padding: 0;
  font-weight: 400;
  color: inherit;
  font-size: inherit;
  font-family: inherit;
  line-height: inherit;
}

.tina-tailwind [data-slate-editor='true'] pre code::before {
  content: none;
}

.tina-tailwind [data-slate-editor='true'] pre code::after {
  content: none;
}

.tina-tailwind [data-slate-editor='true'] table {
  width: 100%;
  table-layout: auto;
  text-align: left;
  margin-top: 2em;
  margin-bottom: 2em;
  font-size: 0.875em;
  line-height: 1.7142857;
}

.tina-tailwind [data-slate-editor='true'] thead {
  color: #252336;
  font-weight: 600;
  border-bottom-width: 1px;
  border-bottom-color: #b2adbe;
}

.tina-tailwind [data-slate-editor='true'] thead th {
  vertical-align: bottom;
  padding-right: 0.5714286em;
  padding-bottom: 0.5714286em;
  padding-left: 0.5714286em;
}

.tina-tailwind [data-slate-editor='true'] tbody tr {
  border-bottom-width: 1px;
  border-bottom-color: #E1DDEC;
}

.tina-tailwind [data-slate-editor='true'] tbody tr:last-child {
  border-bottom-width: 0;
}

.tina-tailwind [data-slate-editor='true'] tbody td {
  vertical-align: top;
  padding-top: 0.5714286em;
  padding-right: 0.5714286em;
  padding-bottom: 0.5714286em;
  padding-left: 0.5714286em;
}

.tina-tailwind [data-slate-editor='true'] {
  font-size: 1rem;
  line-height: 1.75;
}

.tina-tailwind [data-slate-editor='true'] p {
  margin-top: 1.25em;
  margin-bottom: 1.25em;
}

.tina-tailwind [data-slate-editor='true'] img {
  margin-top: 2em;
  margin-bottom: 2em;
}

.tina-tailwind [data-slate-editor='true'] video {
  margin-top: 2em;
  margin-bottom: 2em;
}

.tina-tailwind [data-slate-editor='true'] figure {
  margin-top: 2em;
  margin-bottom: 2em;
}

.tina-tailwind [data-slate-editor='true'] figure > * {
  margin-top: 0;
  margin-bottom: 0;
}

.tina-tailwind [data-slate-editor='true'] h2 code {
  font-size: 0.875em;
}

.tina-tailwind [data-slate-editor='true'] h3 code {
  font-size: 0.9em;
}

.tina-tailwind [data-slate-editor='true'] ol {
  margin-top: 1.25em;
  margin-bottom: 1.25em;
}

.tina-tailwind [data-slate-editor='true'] ul {
  margin-top: 1.25em;
  margin-bottom: 1.25em;
}

.tina-tailwind [data-slate-editor='true'] li {
  margin-top: 0.5em;
  margin-bottom: 0.5em;
}

.tina-tailwind [data-slate-editor='true'] > ul > li p {
  margin-top: 0.75em;
  margin-bottom: 0.75em;
}

.tina-tailwind [data-slate-editor='true'] > ul > li > *:first-child {
  margin-top: 1.25em;
}

.tina-tailwind [data-slate-editor='true'] > ul > li > *:last-child {
  margin-bottom: 1.25em;
}

.tina-tailwind [data-slate-editor='true'] > ol > li > *:first-child {
  margin-top: 1.25em;
}

.tina-tailwind [data-slate-editor='true'] > ol > li > *:last-child {
  margin-bottom: 1.25em;
}

.tina-tailwind [data-slate-editor='true'] ul ul, .tina-tailwind [data-slate-editor='true'] ul ol, .tina-tailwind [data-slate-editor='true'] ol ul, .tina-tailwind [data-slate-editor='true'] ol ol {
  margin-top: 0.75em;
  margin-bottom: 0.75em;
}

.tina-tailwind [data-slate-editor='true'] ul ul, .tina-tailwind [data-slate-editor='true'] ul ol, .tina-tailwind [data-slate-editor='true'] ol ul, .tina-tailwind [data-slate-editor='true'] ol ol {
  margin-top: 0.75em;
  margin-bottom: 0.75em;
}

.tina-tailwind [data-slate-editor='true'] ul ul, .tina-tailwind [data-slate-editor='true'] ul ol, .tina-tailwind [data-slate-editor='true'] ol ul, .tina-tailwind [data-slate-editor='true'] ol ol {
  margin-top: 0.75em;
  margin-bottom: 0.75em;
}

.tina-tailwind [data-slate-editor='true'] ul ul, .tina-tailwind [data-slate-editor='true'] ul ol, .tina-tailwind [data-slate-editor='true'] ol ul, .tina-tailwind [data-slate-editor='true'] ol ol {
  margin-top: 0.75em;
  margin-bottom: 0.75em;
}

.tina-tailwind [data-slate-editor='true'] hr + * {
  margin-top: 0;
}

.tina-tailwind [data-slate-editor='true'] h2 + * {
  margin-top: 0;
}

.tina-tailwind [data-slate-editor='true'] h3 + * {
  margin-top: 0;
}

.tina-tailwind [data-slate-editor='true'] h4 + * {
  margin-top: 0;
}

.tina-tailwind [data-slate-editor='true'] thead th:first-child {
  padding-left: 0;
}

.tina-tailwind [data-slate-editor='true'] thead th:last-child {
  padding-right: 0;
}

.tina-tailwind [data-slate-editor='true'] tbody td:first-child {
  padding-left: 0;
}

.tina-tailwind [data-slate-editor='true'] tbody td:last-child {
  padding-right: 0;
}

.tina-tailwind [data-slate-editor='true'] > :first-child {
  margin-top: 0;
}

.tina-tailwind [data-slate-editor='true'] > :last-child {
  margin-bottom: 0;
}

.tina-tailwind .with-toolbar [data-slate-editor='true'] {
    min-height: 100px;
  }

.first\:pt-4:first-child {
  padding-top: 16px !important;
}

.last\:pb-4:last-child {
  padding-bottom: 16px !important;
}

.hover\:border-gray-200:hover {
  --tw-border-opacity: 1 !important;
  border-color: rgba(225, 221, 236, var(--tw-border-opacity)) !important;
}

.hover\:bg-gray-50:hover {
  --tw-bg-opacity: 1 !important;
  background-color: rgba(246, 246, 249, var(--tw-bg-opacity)) !important;
}

.hover\:bg-blue-600:hover {
  --tw-bg-opacity: 1 !important;
  background-color: rgba(5, 116, 228, var(--tw-bg-opacity)) !important;
}

.hover\:bg-white:hover {
  --tw-bg-opacity: 1 !important;
  background-color: rgba(255, 255, 255, var(--tw-bg-opacity)) !important;
}

.hover\:bg-gray-900:hover {
  --tw-bg-opacity: 1 !important;
  background-color: rgba(37, 35, 54, var(--tw-bg-opacity)) !important;
}

.hover\:bg-gray-100:hover {
  --tw-bg-opacity: 1 !important;
  background-color: rgba(237, 236, 243, var(--tw-bg-opacity)) !important;
}

.hover\:text-blue-500:hover {
  --tw-text-opacity: 1 !important;
  color: rgba(0, 132, 255, var(--tw-text-opacity)) !important;
}

.hover\:text-blue-400:hover {
  --tw-text-opacity: 1 !important;
  color: rgba(34, 150, 254, var(--tw-text-opacity)) !important;
}

.hover\:text-red-500:hover {
  --tw-text-opacity: 1 !important;
  color: rgba(239, 68, 68, var(--tw-text-opacity)) !important;
}

.hover\:text-gray-900:hover {
  --tw-text-opacity: 1 !important;
  color: rgba(37, 35, 54, var(--tw-text-opacity)) !important;
}

.hover\:opacity-100:hover {
  opacity: 1 !important;
}

.hover\:shadow-md:hover {
  --tw-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06) !important;
  box-shadow: var(--tw-ring-offset-shadow, 0 0 #0000), var(--tw-ring-shadow, 0 0 #0000), var(--tw-shadow) !important;
}

.focus\:z-10:focus {
  z-index: 10 !important;
}

.focus\:border-blue-500:focus {
  --tw-border-opacity: 1 !important;
  border-color: rgba(0, 132, 255, var(--tw-border-opacity)) !important;
}

.focus\:text-gray-900:focus {
  --tw-text-opacity: 1 !important;
  color: rgba(37, 35, 54, var(--tw-text-opacity)) !important;
}

.focus\:text-blue-500:focus {
  --tw-text-opacity: 1 !important;
  color: rgba(0, 132, 255, var(--tw-text-opacity)) !important;
}

.focus\:shadow-outline:focus {
  --tw-shadow: 0 0 0 3px rgba(66, 153, 225, 0.5) !important;
  box-shadow: var(--tw-ring-offset-shadow, 0 0 #0000), var(--tw-ring-shadow, 0 0 #0000), var(--tw-shadow) !important;
}

.focus\:outline-none:focus {
  outline: 2px solid transparent !important;
  outline-offset: 2px !important;
}

.focus\:ring-2:focus {
  --tw-ring-offset-shadow: var(--tw-ring-inset) 0 0 0 var(--tw-ring-offset-width) var(--tw-ring-offset-color) !important;
  --tw-ring-shadow: var(--tw-ring-inset) 0 0 0 calc(2px + var(--tw-ring-offset-width)) var(--tw-ring-color) !important;
  box-shadow: var(--tw-ring-offset-shadow), var(--tw-ring-shadow), var(--tw-shadow, 0 0 #0000) !important;
}

.focus\:ring-1:focus {
  --tw-ring-offset-shadow: var(--tw-ring-inset) 0 0 0 var(--tw-ring-offset-width) var(--tw-ring-offset-color) !important;
  --tw-ring-shadow: var(--tw-ring-inset) 0 0 0 calc(1px + var(--tw-ring-offset-width)) var(--tw-ring-color) !important;
  box-shadow: var(--tw-ring-offset-shadow), var(--tw-ring-shadow), var(--tw-shadow, 0 0 #0000) !important;
}

.focus\:ring-blue-500:focus {
  --tw-ring-opacity: 1 !important;
  --tw-ring-color: rgba(0, 132, 255, var(--tw-ring-opacity)) !important;
}

.focus\:ring-indigo-500:focus {
  --tw-ring-opacity: 1 !important;
  --tw-ring-color: rgba(99, 102, 241, var(--tw-ring-opacity)) !important;
}

.focus\:ring-offset-2:focus {
  --tw-ring-offset-width: 2px !important;
}

.focus\:ring-offset-gray-100:focus {
  --tw-ring-offset-color: #EDECF3 !important;
}

.group:hover .group-hover\:text-blue-400 {
  --tw-text-opacity: 1 !important;
  color: rgba(34, 150, 254, var(--tw-text-opacity)) !important;
}

.group:hover .group-hover\:opacity-90 {
  opacity: .9 !important;
}

.group:hover .group-hover\:opacity-100 {
  opacity: 1 !important;
}

@media (min-width: 640px) {

  .sm\:text-sm {
    font-size: 14px !important;
    line-height: 1.43 !important;
  }
}
